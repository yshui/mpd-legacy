...
ffado_configure(struct mpd_ffado_device *fd, struct audio_format *audio_format)
{
	...
	for (int i = 0; i < num_streams; ++i) {
		...
		int ret = ffado_configure_stream(fd->dev, stream);
		if (ret != MPD_SUCCESS)
			return ret;
	}
...
ffado_open(struct audio_output *ao, struct audio_format *audio_format)
{
	...
	int ret = ffado_configure(fd, audio_format);
	if (ret != MPD_SUCCESS) {
...
ffado_play(struct audio_output *ao, const void *chunk, size_t size)
{
	...
	if (fd->buffer_position >= fd->period_size) {
		...
		default:
			log_err("ffado_streaming_wait() failed");
			return 0;
		}
	...
	if (fd->buffer_position >= fd->period_size &&
		...
		log_err("ffado_streaming_transfer_playback_buffers() failed");
		return 0;
	}
